...
## Not buil-in modules. Will be called later, whenever needed.        ##
...
parser = OptionParser()
parser.add_option('-a', '--software', dest = 'software', help = 'Package\'s name the data files come from. Default: Sire.', default = 'Sire')
...
parser.add_option('-i', '--threshold', dest = 'uncorr_threshold', help = 'Perform the analysis with rather all the data if the number of uncorrelated samples is found to be less than this number. If 0 is given, the time series analysis will not be performed at all.', default = 0, type=int)
...
parser.add_option('-p', '--prefix', dest = 'prefix', help = 'Prefix for datafile sets, i.e.\'actual_grad_\' (default).', default = 'actual_grad_')
parser.add_option('-q', '--suffix', dest = 'suffix', help = 'Suffix for datafile sets, i.e. \'dat\' (default).', default = 'dat')
...
parser.add_option('-u', '--units', dest = 'units', help = 'Units to report energies: \'kJ\', \'kcal\', and \'kBT\'. Default: \'kJ\'', default = 'kcal')
...
def getMethods(string):
   ...
   methods     = ['TI','TI-CUBIC','DEXP','IEXP','BAR','MBAR'] if not P.software=='Sire' else ['TI','TI-CUBIC']
   if not string:
      return methods
...
def checkUnitsAndMore(units):
   ...
   if units == 'kJ':
      beta_report = beta/4.184**(P.software=='Sire')
      units = '(kJ/mol)'
   elif units == 'kcal':
      beta_report = 4.184**(P.software!='Sire')*beta
      units = '(kcal/mol)'
   elif units == 'kBT':
      beta_report = 1
      units = '(k_BT)'
...
def uncorrelate(sta, fin, do_dhdl=False):
   ...
   if (not P.uncorr_threshold and P.software=='Sire'):
...
if __name__ == "__main__":
   ...
   # Plot figures.
   if (numpy.array([P.bForwrev, P.breakdown, P.bCFM])==True).any():
